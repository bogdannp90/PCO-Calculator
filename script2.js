...
// ... (codul pentru Fraction și gcd rămâne aici) ...
function gcd(a, b) {
    a = Math.abs(a);
    b = Math.abs(b);
    while (b) {
...
function setupMatrixInputs() {
    ...
    headerRow.insertCell().outerHTML = "<th class='header-diagonal'></th>"; 
...
function afiseazaMatriceJocHtml(gameMatrixStrings) {
    ...
    let html = `<h4>Matricea de Joc Introdusă (Payoff pentru A)</h4><table><thead><tr>`;
    html += `<th class='header-diagonal'></th>`; 
    for (let j = 0; j < n; j++) {
        html += `<th>b<sub>${j + 1}</sub></th>`;
    }
    html += `<th class="summary-column">α = MIN linii</th></tr></thead><tbody>`;

    for (let i = 0; i < m; i++) {
        html += `<tr><th>a<sub>${i + 1}</sub></th>`;
        for (let j = 0; j < n; j++) {
            html += `<td>${new Fraction(gameMatrixStrings[i][j]).toString()}</td>`;
        }
        html += `<td class="summary-column">${new Fraction(alpha[i].toString()).toString()}</td></tr>`;
    }
    
    html += `<tr><th>β=MAX col</th>`; 
    for (let j = 0; j < n; j++) {
        html += `<td>${new Fraction(beta[j].toString()).toString()}</td>`;
    }
    html += `<td class="v1v2-diagonal-cell summary-column">
    ...
    if (Math.abs(v1 - v2) < 1e-9) {
        appendHtml(`<p style="color: green; margin-top: 5px;">Punct șa găsit: V = ${formatNumberDisplay(v1)}. Jocul are o soluție în strategii pure.</p>`);
    } else {
        appendHtml(`<p style="margin-top: 5px;">V<sub>1</sub> (${formatNumberDisplay(v1)}) ≠ V<sub>2</sub> (${formatNumberDisplay(v2)}). Nu există punct șa. Se caută soluții în strategii mixte. Valoarea jocului V aparține intervalului [${formatNumberDisplay(v1)}, ${formatNumberDisplay(v2)}].</p>`);
    }
    return { v1: v1, v2: v2 };
}
...
function generateSimplexIterationHtml(
    iterationLabel, 
    c_objective_coeffs, 
    numDecisionVars, 
    numSlackVars, 
    ...
    let html = `<h4>Tabelul Simplex: ${iterationLabel}</h4>`;
    html += `<table><thead>`;

    if (isInitialTableau) {
        html += `<tr>`;
        html += `<th colspan="3" style="text-align:center; font-size:1.1em; background-color: #6c757d; color:white; border-bottom: 1px solid #5a6268; vertical-align:middle;">MAX</th>`;
        for (let j = 0; j < numDecisionVars; j++) {
            html += `<th style="background-color: #6c757d; color:white; vertical-align:middle; border-bottom: 1px solid #5a6268;">y<sub>${j + 1}</sub></th>`;
        }
        for (let j = 0; j < numSlackVars; j++) {
            html += `<th style="background-color: #6c757d; color:white; vertical-align:middle; border-bottom: 1px solid #5a6268;">U<sub>${j + 1}</sub></th>`;
        }
        html += `</tr>`; 

        html += `<tr>`;
        html += `<th colspan="3" style="background-color: #e9ecef;"></th>`; 
        for (let j = 0; j < c_objective_coeffs.length; j++) {
            html += `<th style="background-color: #e9ecef;">${c_objective_coeffs[j].toString()}</th>`;
        }
        html += `</tr>`;
    }
    ...
    if (isInitialTableau) {
        ...
        let iterSuffix = iterationLabel.replace(/I<sub>|<\/sub>/g, '');
        html += `<tr><td colspan="3" style="text-align:right;">Z<sup>${iterSuffix}</sup> = ${z_value.toString()}</td>`;
    }
...
// NOUĂ FUNCȚIE: afiseazaInterpretareRezultate
function afiseazaInterpretareRezultate(x_optimal_strategies, y_optimal_strategies, V_game, numRows_A, numCols_B) {
    ...
    for (let i = 0; i < numRows_A; i++) {
        const probValue = x_optimal_strategies[i].valueOf();
        const percentage = Math.round(probValue * 100);
        if (probValue > 1e-9) { // Afișează doar strategiile cu probabilitate nenulă
            html += `<li>Strategia a<sub>${i + 1}</sub> ar trebui aplicată cu probabilitatea ${x_optimal_strategies[i].toString()} ( ${percentage}% )</li>`;
        }
    }
    html += `</ul>`;
    ...
    for (let j = 0; j < numCols_B; j++) {
        const probValue = y_optimal_strategies[j].valueOf();
        const percentage = Math.round(probValue * 100);
         if (probValue > 1e-9) { // Afișează doar strategiile cu probabilitate nenulă
            html += `<li>Strategia b<sub>${j + 1}</sub> ar trebui aplicată cu probabilitatea ${y_optimal_strategies[j].toString()} ( ${percentage}% )</li>`;
        }
    }
    html += `</ul>`;
...
function rezolvaPLB_js(gameMatrixStrings, v1Initial, v2Initial) {
    ...
    while (iteration < MAX_ITERATIONS) {
        ...
        for (let j = 0; j < totalVars_LP; j++) {
            if (cj_zj_vals[j].valueOf() > max_cj_zj_val.valueOf() && cj_zj_vals[j].valueOf() > 1e-9) { 
        ...
        appendHtml(generateSimplexIterationHtml(
            iterationLabelForText,
            c_objective,
            numDecisionVars_B,
            numSlackVars_LP,
            totalVars_LP, 
        ...
        if (pivotCol_0_indexed_for_current_step !== -1) {
            ...
            appendHtml(`<p style="margin-top:5px; margin-bottom:15px;"><b>Analiza Iterației ${iterationLabelForText.replace(/I<sub>|<\/sub>/g, '')}:</b> `+
                       `Există &Delta;<sub>k</sub> > 0 (max &Delta;<sub>k</sub> = ${max_cj_zj_val.toString()} pentru variabila ${enteringVarOriginalName}/${enteringVarName_ak}). Se continuă algoritmul.</p>`);
            ...
            if (pivotRow_0_indexed_for_current_step !== -1) {
                ...
                appendHtml(`<p style="margin-bottom:15px;">Variabila ${enteringVarOriginalName} (coloana ${enteringVarName_ak}) intră în bază. ` +
                           `Variabila ${leavingVarOriginalName} (din rândul bazei ${leavingVarName_ak}) iese din bază (raport minim = ${minRatio_val.toString()}). ` + // Am scos "pozitiv" pentru a fi mai general, deși algoritmul caută pozitiv
                           `Elementul pivot este ${pivotElementActual.toString()} (la intersecția ${leavingVarName_ak} și ${enteringVarName_ak}).</p>`);
            }
        }

        if (pivotCol_0_indexed_for_current_step === -1) { 
            appendHtml("<p><strong>Toți &Delta;<sub>k</sub> &le; 0. S-a atins soluția optimă. STOP Algoritm.</strong></p>");
            break;
        }

        if (pivotRow_0_indexed_for_current_step === -1 && pivotCol_0_indexed_for_current_step !== -1) { 
            appendHtml("<p><strong>Problemă nelimitată (Unbounded Solution). Nu se poate selecta o variabilă care să iasă din bază.</strong></p>");
    ...
    appendHtml(`<p>MAX(g) = ${g_max_final.toString()} (${formatNumberDisplay(g_max_final.valueOf())})</p>`);
    
    let y_B_solution_aux = {}; // Soluțiile y_j din problema auxiliară
    ...
    appendHtml(`<p>y<sub>B</sub> (soluții auxiliare pentru B) = (${y_B_str_parts.join(', ')})</p>`);

    appendHtml(`<h4>Soluția Problemei PLA (Player A):</h4>`);
    appendHtml(`<p>MIN(f) = ${g_max_final.toString()} (${formatNumberDisplay(g_max_final.valueOf())})</p>`);
    
    let x_A_solution_aux = {}; // Soluțiile x_i din problema auxiliară
    ...
    appendHtml(`<p>x<sub>A</sub> (soluții auxiliare pentru A) = (${x_A_aux_str_parts.join(', ')})</p>`);
    ...
    if (g_max_final.num === 0) {
        ...
        // Apelăm interpretarea chiar dacă V nu poate fi calculat, pentru a arăta strategiile auxiliare
        afiseazaInterpretareRezultate([], [], new Fraction(0), numRows_A, numCols_B, true /* eroareV */);
    ...
    // Apelarea noii funcții de interpretare
...
function runSolver() {
    ...
    if (Math.abs(initialCheck.v1 - initialCheck.v2) < 1e-9) {
        ...
        // Găsim strategiile pure
        let pureStrategyA = -1, pureStrategyB = -1;
        // Căutăm rândul i pentru care min_j(a_ij) = V_saddle
        for(let i=0; i<gameMatrixStrings.length; i++){
            const rowValues = gameMatrixStrings[i].map(s => new Fraction(s).valueOf());
            if(Math.abs(Math.min(...rowValues) - V_saddle.valueOf()) < 1e-9){
                // Am găsit un rând potențial, acum verificăm dacă maximinul acestui rând este V_saddle
                // Și dacă V_saddle este și minimaxul unei coloane din acest rând
                 for(let j=0; j < gameMatrixStrings[0].length; j++) {
                    if (Math.abs(new Fraction(gameMatrixStrings[i][j]).valueOf() - V_saddle.valueOf()) < 1e-9) {
                        // Verificăm dacă e maxim pe coloană (sau minim, depinde de perspectiva)
                        // Este suficient să găsim o celulă a_ij = V_saddle
                        // și min(rândul i) = V_saddle și max(coloana j) = V_saddle (pt matricea lui A)
                        let colMax = -Infinity;
                        for(let r=0; r < gameMatrixStrings.length; r++) {
                            if(new Fraction(gameMatrixStrings[r][j]).valueOf() > colMax) {
                                colMax = new Fraction(gameMatrixStrings[r][j]).valueOf();
                            }
                        }
                        // Pentru ca a_ij să fie punct șa, trebuie ca a_ij să fie minim pe linia sa și maxim pe coloana sa.
                        // V1 = max(min_pe_linie) și V2 = min(max_pe_coloană). Dacă V1=V2=V_saddle
                        // Strategia A: a_i unde min(linia_i) = V_saddle
                        // Strategia B: b_j unde max(coloana_j) = V_saddle
                        // Mai simplu: găsim a_ij = V_saddle care e minim pe linia i și maxim pe coloana j.
                        
                        // Identificarea exactă a strategiilor pure poate fi complexă doar din V1, V2
                        // Dar putem afișa o interpretare generală
                    }
                 }
            }
        }
        ...
        // Pentru a afișa strategiile pure specifice, ar trebui să căutăm elementul șa.
        // Momentan, mesajul este generic.
    } else {
